...
export function ForumCategories({ categories }: { categories: ForumCategory[] }) {
  ...
  return (
    <section className="mb-10">
      <h2 className="font-orbitron text-xs font-bold uppercase tracking-[0.2em] text-gx-red mb-4">
        {t("forum.categories")}
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {categories.map((cat) => (
          <Link
            ...
            className="group relative flex items-start gap-3 p-4 rounded-lg border border-forum-border bg-forum-panel/80 hover:border-gx-red/40 transition-all"
          >
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-rajdhani text-sm font-bold text-white group-hover:text-gx-red transition-colors">
                {cat.name}
              </h3>
