...
export default function ThreadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = use(params);
  const thread = mockThreads.find((th) => th.id === id);

  if (!thread) {
    return (
      <div className="pt-24 pb-16 px-4 text-center">
        <h1 className="text-2xl font-bold text-foreground">{t("forum.threadNotFound")}</h1>
        <p className="mt-2 text-muted-foreground">{t("forum.threadNotFoundDesc")}</p>
        <Link href="/forum" className="mt-4 inline-block text-sm text-gx-red hover:text-red-400">
          {t("shared.backToForum")}
        </Link>
      </div>
    );
  }
