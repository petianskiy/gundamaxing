        ...
        >
          ...
          {build.status === "WIP" && (
            <div className="absolute top-9 right-2.5 z-[6]">
              <span className="px-1.5 py-0.5 rounded text-[7px] font-bold uppercase tracking-widest bg-amber-400/15 border border-amber-400/40 text-amber-400">
                WIP
              </span>
            </div>
          )}
          ...
          {/* Info panel */}
          <div className="absolute bottom-0 left-0 right-0 p-3.5 z-[5]">
            {/* Techniques */}
            {build.techniques.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-1.5">
                {build.techniques.slice(0, 2).map((tech) => (
                  <span key={tech} className="text-[9px] font-semibold tracking-wide uppercase px-1.5 py-0.5 rounded bg-white/10 text-white/55 border border-white/10">
                    {tech}
                  </span>
                ))}
            ...
            <p className="text-[10px] text-white/45 truncate mb-2.5">{build.kitName}</p>
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              >
                <div className="relative w-[18px] h-[18px] rounded-full overflow-hidden shrink-0">
                  <Image src={build.userAvatar} alt={build.username} fill sizes="18px" className="object-cover" />
                </div>
                <span className="text-[11px] text-white/55 truncate">{build.username}</span>
              </Link>

              <div className="flex items-center gap-0.5 shrink-0">
                <button
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); onLike(); }}
                  className="inline-flex items-center gap-0.5 p-2 -m-1 min-w-[36px] justify-center"
                >
                  <Heart className={cn("h-3.5 w-3.5 transition-colors", isLiked ? "fill-red-500 text-red-500" : "text-white/50 group-hover:text-red-400")} />
                  <span className={cn("text-[11px] transition-colors", isLiked ? "text-red-500 font-semibold" : "text-white/50")}>
                ...
                <Link
                  ...
                  onClick={(e) => e.stopPropagation()}
                  className="inline-flex items-center gap-0.5 p-2 -m-1 min-w-[36px] justify-center group/c"
                >
                  <MessageCircle className="h-3.5 w-3.5 text-white/50 group-hover/c:text-blue-400 transition-colors" />
                  <span className="text-[11px] text-white/50">{build.comments}</span>
                </Link>
                <button
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); onBookmark(); }}
                  className="inline-flex items-center p-2 -m-1 group/b"
                >
                  <Bookmark className={cn("h-3.5 w-3.5 transition-colors", isBookmarked ? "fill-white text-white" : "text-white/50 group-hover/b:text-yellow-400")} />
                </button>
              </div>
            </div>
          </div>
